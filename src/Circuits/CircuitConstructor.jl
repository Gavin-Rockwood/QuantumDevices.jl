...
- `interactions`: A collection of typed interaction terms built with `op`, `interaction`, or `coupling`.
- `operators_to_add=Dict{String, Any}()`: (Optional) A dictionary of typed operator expressions to add to the circuit, keyed by operator name.
...
- Builds the bare Hamiltonian (`H_op_0`) and evaluates typed interaction terms to form the full Hamiltonian (`H_op`).
...
function init_circuit(components :: AbstractArray{T1}, 
    ...
    dressed_kwargs = Dict{Symbol, Any}(dressed_kwargs)
    ...
    for interaction in interactions
        H_op += evaluate_interaction(interaction, components; use_sparse = use_sparse)
    end
