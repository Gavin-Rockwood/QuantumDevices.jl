...
function EnsembleTimeEvolutionProblem(
    ...
    function remake_problem(prob, i)
    ...
    ensemble_func(prob, ctx) = remake_problem(prob, ctx.sim_id)
    ensemble_func(prob, i, repeat) = remake_problem(prob, i)
...
function sesolve(prob::EnsembleTimeEvolutionProblem, alg = DE.Tsit5(); backend = DE.EnsembleThreads())
   ...
   to_return = Array{qt.TimeEvolutionSol}(undef, prob.problem_dims)
    for i in 1:length(sols.u)
        sol_i = sols.u[i]
        ψt = map(ϕ -> qt.QuantumObject(ϕ, type = qt.Ket(), dims = prob.prob.dimensions), sol_i.u)
        sol = qt.TimeEvolutionSol(
                    prob.prob.times,
                    sol_i.t,
                    ψt,
                    qt._get_expvals(sol_i, qt.SaveFuncSESolve),
                    sol_i.retcode,
                    sol_i.alg,
                    sol_i.prob.kwargs[:abstol],
                    sol_i.prob.kwargs[:reltol],
                )
