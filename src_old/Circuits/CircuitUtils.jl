"""
    add_operator!(circuit::Circuit, operator::AbstractOperatorExpr, name::String; use_sparse=true)

Adds a custom operator to the given `circuit` object.

# Arguments
- `circuit::Circuit`: The circuit to which the operator will be added.
- `operator::AbstractOperatorExpr`: A typed operator expression built with `op`, algebraic operators, or `coupling`.
...
# Description
Evaluates the typed operator expression against the circuit components and stores the resulting full-circuit operator in `circuit.ops`.
"""
function add_operator!(circuit :: Circuit, operator :: AbstractOperatorExpr, name :: String; use_sparse = true)
    components = [circuit.components[component_name] for component_name in circuit.order]
    circuit.ops[name] = evaluate_operator_expr(operator, components; use_sparse = use_sparse)
...
function get_dressed_states(H0 :: qt.QuantumObject,
    ...
    for δ in δs
        ...
        for interaction in interactions
            H += δ * evaluate_interaction(interaction, components)
        end
