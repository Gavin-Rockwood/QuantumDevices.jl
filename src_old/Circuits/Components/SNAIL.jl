...
module SNAIL_TOOLS
    ...
    function get_c_coeffs_bare(N_val, α_val, Φᵉ_val)
        phase(φ_val) = (φ_val - Φᵉ_val) / N_val
        c1(φ_val) = α_val * sin(φ_val) + sin(phase(φ_val))

        f_to_min(φ_val, p) = [c1(φ_val[1])]

        prob = NLS.NonlinearProblem(f_to_min, [0.0], nothing)
        φ_min = NLS.solve(prob)[1]
        @debug "φ_min: $φ_min"

        return [
            0.0,
            α_val * cos(φ_min) + cos(phase(φ_min)) / N_val,
            -α_val * sin(φ_min) - sin(phase(φ_min)) / N_val^2,
            -α_val * cos(φ_min) - cos(phase(φ_min)) / N_val^3,
            α_val * sin(φ_min) + sin(phase(φ_min)) / N_val^4,
            α_val * cos(φ_min) + cos(phase(φ_min)) / N_val^5,
        ]
    end
