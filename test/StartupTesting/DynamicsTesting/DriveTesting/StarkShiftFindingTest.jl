@testset "Finding Stark Shifts" begin
    ...
    transmon = QD.Circuits.init_components["transmon"](;transmon_params...)
    ...
    resonator = QD.Circuits.init_components["resonator"](;resonator_params...)

    interactions = [
        QD.Circuits.coupling(
            0.026877206812551357,
            QD.Circuits.op(:transmon, :n) * QD.Circuits.op(:resonator, :a_minus_adag_im),
        ),
    ]
    circuit = 0
    operators_to_add = Dict{String, Any}(
        "nt" => QD.Circuits.op(:transmon, :n),
        "a" => QD.Circuits.op(:resonator, :a),
    )
    circuit = QD.Circuits.init_circuit([transmon, resonator], interactions, use_sparse = true, operators_to_add = operators_to_add)
    ...
    stark_shift_res = QD.Dynamics.find_resonance(H0, drive_op, amplitude, base_freq .+ stark_shifts, reference_states)
