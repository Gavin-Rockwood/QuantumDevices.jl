...
function device_fixture()
    ...
    g = QD.DeviceParameter(:g;
        domain = QD.realdomain(), fixed = false, default = 0.05)
...
@testset "QuantumDevice registration" begin
    ...
    @test_throws Exception QD.register!(
        device,
        QD.Component(QD.QubitSpec(4.8), :q1),
    )
...
@testset "QuantumDevice recipe validation" begin
    ...
    other_interaction =
        QD.param(other_g) * QD.op(:q1, :x) * QD.op(:q2, :x)
...
@testset "device modelspec resolution and snapshots" begin
    ...
    @test recipe.parameters[QD.ParamPath(:q1, :frequency)] ===
        q1.spec.frequency
    ...
    collision_recipe = QD.modelspec(
        device,
        :collision,
        (:q1, :q2),
        (:xx,);
        parameters = (colliding_g,),
    )
...
@testset "QuantumDevice numerical conveniences" begin
    ...
    @test norm(from_device.hamiltonian - direct.hamiltonian) < 1e-12
